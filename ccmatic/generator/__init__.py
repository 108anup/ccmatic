...
def value_on_template_execution(
    ...
    if(template_type == template_type.IF_ELSE_COMPOUND_DEPTH_1):
        assert n_expr % 2 == 0
    ...
    if(template_type == TemplateType.IF_ELSE_COMPOUND_DEPTH_1):
        ...
        for i in range(int(n_expr/2)):
            ...
            root_expr = value_on_template_execution(
                TemplateType.IF_ELSE_CHAIN, [c2], [e1, e2])
            root_exprs.append(root_expr)
            if(c1 is not None):
                root_conds.append(c1)
        return value_on_template_execution(
            TemplateType.IF_ELSE_CHAIN, root_conds, root_exprs)
...
def str_on_template_execution(
    ...
    if(template_type == TemplateType.IF_ELSE_COMPOUND_DEPTH_1):
        ...
        for i in range(int(n_expr/2)):
            ...
            root_expr = str_on_template_execution(
                TemplateType.IF_ELSE_CHAIN, [c2], [e1, e2])
            root_exprs.append(root_expr)
            if(c1 is not None):
                root_conds.append(c1)
        return str_on_template_execution(
            TemplateType.IF_ELSE_CHAIN, root_conds, root_exprs)
